...
switch (process.argv[2]) {
  ...
  case "task": //memanggil task
    console.log("Daftar Pekerjaan");
    console.log(
      `${index + 1}. ${bacaData[index].cek} ${bacaData[index].content}`
    );
    process.exit(0);
  ...
  case "tag": //memberi tag atau tanda
    ...
    console.log(
      ` tag '${bacaData[index].tag}' telah ditambahkan ke daftar ${bacaData[index].content}`
    );
    fs.writeFileSync("notepad.json", JSON.stringify(bacaData, null, 3));
    process.exit(0);
}
